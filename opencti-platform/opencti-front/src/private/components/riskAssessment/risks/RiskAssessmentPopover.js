...
class RiskAssessmentPopover extends Component {
  ...
  onPoamSubmit(values, { setSubmitting, resetForm }) {
    const finalValues = R.pipe(
      ...
      R.toPairs,
      R.map((n) => {
        if (values.poam_id === '' || values.poam_id === null) {
          return {
            key: n[0],
            value: this.props.node.poam_id,
            operation: 'remove',
          };
        }
        return {
  ...
  onRiskDeadlineSubmit(values, { setSubmitting, resetForm }) {
    const finalValues = R.pipe(
      ...
      R.map((n) => {
        if (!values.deadline) {
          const date = this.props.node.deadline;
          return {
            key: n[0],
            value: [adaptFieldValue(parse(date))],
            operation: 'remove',
          };
        }
        return {
          ...
          >
            {({
              ...
              {this.props.node.poam_id === null || this.props.node.poam_id === '' ? <DialogTitle>{t('Assign POAM ID')}</DialogTitle> : <DialogTitle>{t('Remove POAM ID')}</DialogTitle>}
                ...
                {values.poam_id && (
                  <DialogContent className={classes.clearContainer} style={{ marginTop: '-13px' }}>
                    <IconButton
                      onClick={() => setFieldValue('poam_id', '')}
            ...
            {({ submitForm, handleReset, isSubmitting }) => (
              <Form style={{ padding: '10px 15px 20px 5px' }}>
                ...
                >
                  ...
                  <DialogActions
                    style={{ marginTop: '6px' }}
                    className={classes.dialogRiskLevelAction}
                  >
                    <Button
                      onClick={handleReset}
                      disabled={this.state.deleting}
          ...
          >
            {({
              ...
              {this.props.node.deadline ? <DialogTitle>{t('Remove Risk Deadline')}</DialogTitle> : <DialogTitle>{t('Assign Risk Deadline')}</DialogTitle>}
                ...
                {values.deadline && (
                  <DialogContent className={classes.clearContainer} style={{ marginTop: '-20px' }}>
                    <IconButton
