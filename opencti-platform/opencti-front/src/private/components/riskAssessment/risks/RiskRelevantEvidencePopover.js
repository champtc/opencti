...
const riskRelevantEvidencePopoverQuery = graphql`
  query RiskRelevantEvidencePopoverQuery($id: ID!) {
...
class RiskRelevantEvidencePopover extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: false,
  ...
  componentDidMount() {
    if (this.props.id) {
      fetchQuery(riskRelevantEvidencePopoverQuery, {
        id: this.props.id,
      })
        .toPromise()
        .then((data) => {
        ...
        >
          {({ submitForm, handleReset, isSubmitting }) => (
            <Dialog open={this.state.open} fullWidth={true} maxWidth='sm'>
              <DialogTitle classes={{ root: classes.dialogTitle }}>{t('Evidence')}</DialogTitle>
...
RiskRelevantEvidencePopover.propTypes = {
