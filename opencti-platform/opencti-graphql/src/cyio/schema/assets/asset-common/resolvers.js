...
const assetCommonResolvers = {
  Query: {
    assetList: async (_, args, { dbName, dataSources, selectMap }) => {
      const sparqlQuery = getSelectSparqlQuery('ASSET', selectMap.getNode('node'), undefined, args.filters);
      const reducer = getReducer('ASSET');
      let response;
      try {
    ...
    asset: async (_, args, { dbName, dataSources, selectMap }) => {
      const sparqlQuery = getSelectSparqlQuery('ASSET', selectMap.getNode('asset'), args.id);
      const reducer = getReducer('ASSET');
      let response;
      try {
    ...
    itAssetList: async (_, args, { dbName, dataSources, selectMap }) => {
      ...
      const sparqlQuery = getSelectSparqlQuery('IT-ASSET', selectList, undefined, args.filters);
      const reducer = getReducer('IT-ASSET');
      let response;
      try {
        response = await dataSources.Stardog.queryAll({
          dbName,
    ...
    itAsset: async (_, args, { dbName, dataSources, selectMap }) => {
      ...
      const sparqlQuery = getSelectSparqlQuery('IT-ASSET', selectList, args.id);
      const reducer = getReducer('IT-ASSET');
      let response;
      try {
        response = await dataSources.Stardog.queryById({
  ...
  Mutation: {
    deleteAsset: async (_, { id }, { dbName, dataSources }) => {
      ...
      await dataSources.Stardog.delete({ dbName, sparqlQuery: dq, queryId: 'Delete Asset' });
  ...
  IpAddressRange: {
    starting_ip_address: async (parent, _, { dbName, dataSources, selectMap }) => {
      if (parent.start_addr_iri === undefined && parent.starting_ip_address !== undefined)
        return parent.starting_ip_address;
      ...
      const sparqlQuery = selectIpAddressByIriQuery(
        parent.start_addr_iri,
        selectList === undefined ? null : selectList
      );
      const response = await dataSources.Stardog.queryById({
        dbName,
        sparqlQuery,
      ...
      if (response === undefined) return [];
      if (Array.isArray(response) && response.length > 0) {
        const reducer = getIpAddrReducer(addrType);
        return reducer(response[0]);
      }
      // Handle reporting Stardog Error
      if (typeof response === 'object' && 'body' in response) {
    ...
    ending_ip_address: async (parent, _, { dbName, dataSources, selectMap }) => {
      if (parent.ending_addr_iri === undefined && parent.ending_ip_address !== undefined)
        return parent.ending_ip_address;
