...
import { selectHardwareByIriQuery } from '../sparql-query.js';


export const findHardwareByIriList = async (parent, iriList, args, dbName, dataSources, select) => {
  let response;
  try {
    response = await selectByBulkIris(iriList, selectHardwareByIriQuery, assetSingularizeSchema, dbName, dataSources, select);
  } catch (e) {
    logApp.error(e);
    throw e;
  }
