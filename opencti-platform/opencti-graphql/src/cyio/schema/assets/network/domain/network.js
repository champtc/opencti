...
import { selectNetworkByIriQuery } from '../sparql-query.js';


export const findNetworkByIriList = async (parent, iriList, args, dbName, dataSources, select) => {
  let response;
  try {
    response = await selectByBulkIris(iriList, selectNetworkByIriQuery, assetSingularizeSchema, dbName, dataSources, select);
  } catch (e) {
    logApp.error(e);
    throw e;
  }
...
export const determineDisplayName = (item) => {
    ...
    if (item.name !== item.network_id) {
      display_name = display_name + ` (${item.network_id})`;
    }
    return display_name;
  }