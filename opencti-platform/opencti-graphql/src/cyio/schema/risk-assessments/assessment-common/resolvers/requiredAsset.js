  ...
  Query: {
    requiredAssets: async (_, args, { dbName, dataSources, selectMap }) => {
      ...
      if (Array.isArray(response) && response.length > 0) {
        ...
        for (const reqAsset of reqAssetList) {
    ...
    createRequiredAsset: async (_, { input }, { dbName, selectMap, dataSources }) => {
      ...
      if (subjects !== undefined && subjects !== null) {
        for (const subject of subjects) {
          ...
          if (result == undefined || result.length === 0)
            throw new CyioError(`Entity does not exist with ID ${subject.subject_ref}`);
      ...
      if (links !== undefined && links.length > 0) {
        const attachQuery = attachToRequiredAssetQuery(id, 'links', links);
        await dataSources.Stardog.create({
          dbName,
          sparqlQuery: attachQuery,
      ...
      if (remarks !== undefined && links.length > 0) {
        const attachQuery = attachToRequiredAssetQuery(id, 'remarks', remarks);
        await dataSources.Stardog.create({
          dbName,
          sparqlQuery: attachQuery,
          ...
          switch (editItem.key) {
            case 'subjects':
              ...
              if (editItem.operation === 'add' || editItem.operation === 'replace') {
                ...
                for (const subject of editObjects) {
                  let result;
                  try {
                  ...
                  if (result == undefined || result.length === 0)
                    throw new CyioError(`Entity does not exist with ID ${obj.subject_ref}`);
                  subject.subject_ref = result[0].iri;

                  // check if there is a corresponding replacement subject already exists
                  try {
                    const args = { filters: [{ key: 'subject_type', values: [subject.subject_type] }] };
              ...
              if (editItem.operation !== 'add') {
                const subjects = `<${response[0][editItem.key]}>`;
                try {
