...
const subjectResolvers = {
      ...
      if (Array.isArray(response) && response.length > 0) {
        ...
        for (const subject of subjectList) {
          ...
          if (!subject.hasOwnProperty('subject_ref') || subject.subject_ref === undefined) {
            console.warn(
              `[CYIO] CONSTRAINT-VIOLATION: (${dbName}) ${subject.iri} missing field 'subject_ref'; skipping`
            );
            skipCount++;
            continue;
          }
          ...
          if (!subject.hasOwnProperty('subject_id') || subject.subject_id === undefined) {
            // logApp.warn(`[CYIO] CONSTRAINT-VIOLATION: (${dbName}) ${subject.iri} missing field 'subject_id'; skipping`);
            console.warn(
              `[CYIO] CONSTRAINT-VIOLATION: (${dbName}) ${subject.iri} missing field 'subject_id'; skipping`
            );
            skipCount++;
            continue;
          }
  ...
  Mutation: {
    createSubject: async (_, { input }, { dbName, selectMap, dataSources }) => {
      ...
      const sparqlQuery = selectObjectIriByIdQuery(input.subject_ref, input.subject_type);
      try {
      ...
      if (result === undefined || result.length === 0)
        throw new CyioError(`Entity does not exist with ID ${input.subject_ref}`);
    ...
    deleteAssessmentSubject: async (_, { id }, { dbName, dataSources }) => {
      ...
      if (subject.include_subjects_iri !== undefined && subject.include_subjects_iri != null) {
        for (const subjectIri of subject.include_subjects_iri) {
          const subQuery = deleteSubjectByIriQuery(subjectIri);
          try {
            await dataSources.Stardog.delete({
              dbName,
      ...
      if (subject.exclude_subjects_iri !== undefined && subject.exclude_subjects_iri != null) {
        for (const subjectIri of subject.exclude_subjects_iri) {
          const subQuery = deleteSubjectByIriQuery(subjectIri);
          try {
            await dataSources.Stardog.delete({
              dbName,
  ...
  Subject: {
    ...
    subject_ref: async (parent, _, { dbName, dataSources, selectMap }) => {
      ...
      if (Array.isArray(parent.subject_ref_iri) && parent.subject_ref_iri.length > 1) {
        console.log(
          `[CYIO] CONSTRAINT-VIOLATION: ${parent.iri} 'subject_ref' violates maxCount constraint; dropping extras`
        );
      }

      const reducer = getCommonReducer(parent.subject_type.toUpperCase());
      let iri;
      ...
      if (parent.hasOwnProperty('subject_id') && parent.hasOwnProperty('subject_name')) {
        const subjectRef = {
          ...
          name:
            parent.subject_version !== undefined
              ? `${parent.subject_name} ${parent.subject_version}`
              : `${parent.subject_name}`,
        };
        ...
        if (parent.hasOwnProperty('subject_location_type')) {
          if (parent.subject_location_type === 'data_center') parent.subject_location_type = 'data-center';
          subjectRef.location_type = parent.subject_location_type;
        }
      ...
      if (Array.isArray(response) && response.length > 0) {
        if (response[0].id === undefined) {
          console.log(
            `[CYIO] CONSTRAINT-VIOLATION: (${dbName}) ${response[0].iri} required field 'id' is missing; skipping`
          );
          return null;
        }
  ...
  AssessmentSubject: {
    ...
    include_subjects: async (parent, _, { dbName, dataSources, selectMap }) => {
      ...
      const results = [];
      const reducer = getReducer('SUBJECT');
      const sparqlQuery = selectAllSubjects(selectMap.getNode('include_subjects'), undefined, parent);
      let response;
      try {
        response = await dataSources.Stardog.queryById({
          dbName,
    ...
    exclude_subjects: async (parent, _, { dbName, dataSources, selectMap }) => {
      ...
      const results = [];
      const reducer = getReducer('SUBJECT');
      const sparqlQuery = selectAllSubjects(selectMap.getNode('exclude_subjects'), undefined, parent);
      let response;
      try {
        response = await dataSources.Stardog.queryById({
          dbName,
