...
import {
  getReducer,
  getControlObjectiveSelectionIri,
...
} from '../schema/sparql/controlObjectiveSelection.js';
import { createControlObjective, deleteControlObjectiveByIri } from './controlObjective.js'
...
export const findControlObjectiveSelectionById = async (id, dbName, dataSources, select) => {
  ...
  let iri = getControlObjectiveSelectionIri(id);
...
export const createControlObjectiveSelection = async (input, dbName, dataSources, select) => {
  ...
  let nestedDefinitions = {
    'include_objectives': { values: input.include_objectives, props: {}, objectType: 'include_objectives', createFunction: createControlObjective },
    'exclude_objectives': { values: input.exclude_objectives, props: {}, objectType: 'exclude_objectives', createFunction: createControlObjective },
  };
  ...
  await processNestedDefinitions(id, nestedDefinitions, dbName, dataSources, attachToControlObjectiveSelectionQuery);
...
export const deleteControlObjectiveSelectionById = async ( id, dbName, dataSources) => {
  let select = ['iri','id', 'include_objectives', 'exclude_objectives'];
  let idArray = [];
  if (!Array.isArray(id)) {
    idArray = [id];
  } else {
  ...
  for (let itemId of idArray) {
    ...
    let nestedReferences = {
      'include_objectives': { iris: response[0].include_objectives, deleteFunction: deleteControlObjectiveByIri},
      'exclude_objectives': { iris: response[0].exclude_objectives, deleteFunction: deleteControlObjectiveByIri},
    };
...
export const editControlObjectiveSelectionById = async (id, input, dbName, dataSources, select, schema) => {
    ...
    for (value of editItem.value) {
      switch(editItem.key) {
        case 'include_objectives':
          objectType = 'include_objectives';
          fieldType = 'id';
          break;
        case 'exclude_objectives':
          objectType = 'exclude_objectives';
          fieldType = 'id';
          break;
        default:
          fieldType = 'simple';
  ...
  const query = updateQuery(
    getControlObjectiveSelectionIri(id),
...
export const attachToControlObjectiveSelection = async (id, field, entityId, dbName, dataSources) => {
  ...
  let attachableObjects = {
    ...
    'exclude_objectives': 'exclude_objectives',
  }
