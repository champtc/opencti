...
import { createSelectedControl, deleteSelectedControlByIri } from '../domain/selectedControl.js'
...
export const findControlSelectionById = async (id, dbName, dataSources, select) => {
  ...
  let iri = getControlSelectionIri(id);
...
export const createControlSelection = async (input, dbName, dataSources, select) => {
  ...
  let nestedDefinitions = {
    'include_controls': { values: input.include_controls, props: {}, objectType: 'include_controls', createFunction: createSelectedControl },
    'exclude_controls': { values: input.exclude_controls, props: {}, objectType: 'exclude_controls', createFunction: createSelectedControl },
  };
  ...
  await processNestedDefinitions(id, nestedDefinitions, dbName, dataSources, attachToControlSelectionQuery);
...
export const deleteControlSelectionById = async ( id, dbName, dataSources) => {
  let select = ['iri','id', 'include_controls', 'exclude_controls'];
  let idArray = [];
  if (!Array.isArray(id)) {
    idArray = [id];
  } else {
  ...
  for (let itemId of idArray) {
    ...
    let nestedReferences = {
      'include_controls': { iris: response[0].include_controls, deleteFunction: deleteSelectedControlByIri},
      'exclude_controls': { iris: response[0].exclude_controls, deleteFunction: deleteSelectedControlByIri},
    };
...
export const editControlSelectionById = async (id, input, dbName, dataSources, select, schema) => {
  ...
  for (let editItem  of input) {
    ...
    for (value of editItem.value) {
      switch(editItem.key) {
        case 'include_controls':
          objectType = 'include_controls';
          fieldType = 'id';
          break;
        case 'exclude_controls':
          objectType = 'exclude_controls';
          fieldType = 'id';
          break;
        default:
          fieldType = 'simple';
