...
} from '../schema/sparql/selectedControl.js';
...
export const findSelectedControlById = async (id, dbName, dataSources, select) => {
  ...
  let iri = getSelectedControlIri(id);
...
export const createSelectedControl = async (input, dbName, dataSources, select) => {
  ...
  let objectReferences = {
    ...
    'statements': { ids: input.statements, objectType: 'statements' },
  };
  if (input.control) delete input.control;
  if (input.statements) delete input.statements;
  ...
  for (let [key, value] of Object.entries(objectReferences)) {
    ...
    if (iris.length > 0) {
      // attach the definition to the new selected control
      let attachQuery = attachToSelectedControlQuery(id, key, iris );
      try {
        response = await dataSources.Stardog.create({
          dbName,
          sparqlQuery: attachQuery,
          queryId: `Attaching one or more ${itemName} to selected control`
          });
      } catch (e) {
        console.log(e)
        throw e
