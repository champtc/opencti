...
export const resultReducer = (item) => {
  ...
  return {
    ...
    ...(item.ingested && { ingested: item.ingested }),
    ...
    ...(item.assets && { asset_iris: item.assets }),
    ...(item.components_discovered && { component_iris: item.components_discovered }),
    ...(item.inventory_items_discovered && { inventory_item_iris: item.inventory_items_discovered }),
...
export const getResultIri = (id) => {
  ...
  return `<http://csrc.nist.gov/ns/oscal/assessment-results#Result-${id}>`
}

// Utilities - Result Local Definitions
export const generateResultLocalDefinitionsId = (input) => {
  const id = generateId( );
  return id;
}

export const getResultLocalDefinitionsIri = (id) => {
  // ensure the id is a valid UUID
  if (!checkIfValidUUID(id)) throw new UserInputError(`Invalid identifier: ${id}`);

  // TODO: Need to clean up data to use common IRI format
  // return `<http://cyio.darklight.ai/result--${id}>`;
  return `<http://csrc.nist.gov/ns/oscal/assessment-results#ResultLocalDefinitions-${id}>`
}


// Query Builders - Result
export const selectResultQuery = (id, select) => {
  return selectResultByIriQuery(getResultIri(id), select);
}
...
export const selectResultByIriQuery = (iri, select) => {
  ...
  // Handle gathering assets
  if (select.includes('components_discovered') || select.includes('inventory_items_discovered')) {
    select.push('assets');
    // remove so we only retrieve them once
    select = select.filter((i) => i !== 'components_discovered');
    select = select.filter((i) => i !== 'inventory_items_discovered');
  }
  ...
  if (parent !== undefined && parent.iri !== undefined) {
    ...
    constraintClause = `{
      ...
      WHERE {
          ...
          <http://csrc.nist.gov/ns/oscal/assessment-results#results> ?iri .
      }
    }`;
  }
...
export const insertResultQuery = (propValues) => {
  ...
  INSERT DATA {
    GRAPH ${iri} {
      ${iri} a <http://darklight.ai/ns/oscal/assessment-results#Result> .
      ${iri} a <http://csrc.nist.gov/ns/oscal/assessment-results#Result> .
...
export const attachToResultLocalDefinitionsQuery = (id, field, itemIris) => {
  if (!resultPredicateMap.hasOwnProperty(field)) return null;
  const iri = getResultLocalDefinitionsIri(id);
  const predicate = resultPredicateMap[field].predicate;
...
export const resultPredicateMap = {
  ...
  ingested: {
    predicate: "<http://darklight.ai/ns/oscal/assessment-results#ingested>",
    binding: function (iri, value) { return parameterizePredicate(iri, value ? `"${value}"^^xsd:dateTime` : null, this.predicate, "ingested");},
    optional: function (iri, value) { return optionalizePredicate(this.binding(iri, value));},
  },
  ingest_status: {
  ...
  assets: {
    ...
    binding: function (iri, value) { return parameterizePredicate(iri, value ? `"${value}"` : null,  this.predicate, "assets");},
    optional: function (iri, value) { return optionalizePredicate(this.binding(iri, value));},
  },
  components_discovered: {
    predicate: "<http://darklight.ai/ns/oscal/assessment-results#assets>",
    binding: function (iri, value) { return parameterizePredicate(iri, value ? `"${value}"` : null,  this.predicate, "components_discovered");},
    optional: function (iri, value) { return optionalizePredicate(this.binding(iri, value));},
  },
  inventory_items_discovered: {
    predicate: "<http://darklight.ai/ns/oscal/assessment-results#assets>",
    binding: function (iri, value) { return parameterizePredicate(iri, value ? `"${value}"` : null,  this.predicate, "inventory_items_discovered");},
    optional: function (iri, value) { return optionalizePredicate(this.binding(iri, value));},
  },
};
...
export const resultSingularizeSchema = { 
