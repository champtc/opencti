...
import {
  ...
  selectProblemTypeByIriQuery,
  selectAllProblemTypesQuery,
  deleteProblemTypeQuery,
  deleteProblemTypeByIriQuery,
  attachToProblemTypeQuery,
  detachFromProblemTypeQuery,
} from '../schema/sparql/problemType.js';


export const findProblemTypeById = async (id, dbName, dataSources, select) => {
  // ensure the id is a valid UUID
  if (!checkIfValidUUID(id)) throw new UserInputError(`Invalid identifier: ${id}`, {identifier: `${id}`});

  let iri = getProblemTypeIri(id);
  return findProblemTypeByIri(iri, dbName, dataSources, select);
};

export const findProblemTypeByIri = async (iri, dbName, dataSources, select) => {
  const sparqlQuery = selectProblemTypeByIriQuery(iri, select);
  let response;
...
export const findAllProblemTypes = async ( parent, args, dbName, dataSources, select ) => {
  let response;

  const sparqlQuery = selectAllProblemTypesQuery(select, args, parent);
  try {
    response = await dataSources.Stardog.queryAll({
      dbName: dbName,
      sparqlQuery,
      queryId: "Select List of Problem Types",
      singularizeSchema: singularizeProblemTypeSchema
    });
  } catch (e) {
    logApp.error(e);
...
export const deleteProblemTypeById = async ( id, dbName, dataSources ) => {
  ...
  for (let itemId of idArray) {
    ...
    iri = await deleteProblemTypeByIri(getProblemTypeIri(itemId),  dbName, dataSources);
    if (iri) {
...
export const deleteProblemTypeByIri = async ( iri, dbName, dataSources ) => {
  let select = ['iri','id'];

  // check if object with iri exists
  let problemType = await problemTypeExistsByIri(iri, select, dbName, dataSources);
  if (problemType == undefined || problemType == null) return null;

  let sparqlQuery = deleteProblemTypeByIriQuery(iri);
  try {
...
export const attachToProblemType = async (id, field, entityId, dbName, dataSources) => {
  ...
  sparqlQuery = selectProblemTypeByIriQuery(iri, select);
...
export const problemTypeExists = async (checkId, select, dbName, dataSources) => {
  // ensure the id is a valid UUID
  ...
  return problemTypeExistsByIri(getProblemTypeIri(checkId), select, dbName, dataSources);
};

// check if object with iri exists
export const problemTypeExistsByIri = async (iri, select, dbName, dataSources) => {
  let selectQuery = selectProblemTypeByIriQuery(iri, select);
  let problemType;
