...
const taxonomyEntryReducer = (item) => {
  ...
  if (item.object_type === undefined) {
    ...
    if (item.iri.includes('taxonmy-map-entry')) item.object_type = 'taxonmy-map-entry';
}
...
export const insertTaxonomyEntryQuery = (propValues) => {
  ...
  const iri = `<http://cyio.darklight.ai/taxonmy-map-entry--${id}>`;
  ...
  INSERT DATA {
    GRAPH ${iri} {
      ...
      ${iri} <http://darklight.ai/ns/common#object_type> "taxonmy-map-entry" . 
...
export const selectTaxonomyEntryQuery = (id, select) => {
  return selectTaxonomyEntryQueryByIriQuery(`<http://cyio.darklight.ai/taxonmy-map-entry--${id}>`, select);
}
